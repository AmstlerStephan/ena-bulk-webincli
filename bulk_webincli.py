...
columns_to_include = ["sample", "study", "instrument_model", "library_name", "library_source", "library_selection", "library_strategy", "file_name"]
...
def get_args():
    ...
    parser.add_argument(
        "--platform",
        help="Sequencing platform: ILLUMINA, PACBIO_SMRT, OXFORD_NANOPORE, BGISEQ, LS454, ION_TORRENT, CAPILLARY, DNBSEQ, ELEMENT, ULTIMA, VELA_DIAGNOSTICS, GENAPSYS, GENEMIND, TAPESTRI",
        choices=["ILLUMINA", "PACBIO_SMRT", "OXFORD_NANOPORE", "BGISEQ", "LS454", "ION_TORRENT", "CAPILLARY", "DNBSEQ", "ELEMENT", "ULTIMA", "VELA_DIAGNOSTICS", "GENAPSYS", "GENEMIND", "TAPESTRI"],
        type=str,
        required=True,
    )
    parser.add_argument(
        "--name",
        help="name of the run: unique identifier",
        type=str,
        required=True,
    )
    parser.add_argument(
    ...
    if args.parallel is None:
        args.parallel = False
    elif not 0 < args.parallel <= 30:
        print("> ERROR: Invalid number of cores/threads provided. This value should be between 1 and 30 (inclusive).")
        sys.exit()
...
class GenerateManifests:
    ...
    def __init__(self, df, directory, context, platform, name):
        self.df = df
        self.directory = directory
        self.context = context  # The context that Webin-CLI is to be used in (e.g. reads)
        self.platform = platform # sequencing platform
    ...
    def row_processing(self, row):
        ...
        if (self.context == "reads"):  # If reads are being submitted, get the name of the file to obtain a prefix
            prefix_field = row_meta.get("file_name")
        ...
        manifest_file = Path(self.manifest_dir) / "Manifest_{}.tsv".format(prefix)
    ...
    def create_manifest(self, metadata_content):
        ...
        for item in metadata_content.items():
            ...
            if field in columns_to_include:
                ...
                elif field == "file_name":  # Specify the appropriate file type
        ...
        first_col.append(str("name"))
        second_col.append(str(self.name))
        first_col.append(str("platform"))
        second_col.append(str(self.platform))
...
if __name__ == "__main__":
    ...
    create_manifests = GenerateManifests(to_process, args.directory, args.geneticContext, args.platform, args.name)
